#!/usr/bin/env bash
# Update Formula/cashop-console.rb in Cashop-Tech/homebrew-tap to point at a new release.
...
TARBALL_LOCAL="release/cashop-console-cli-${VERSION}.tar.gz"
TARBALL_URL="https://github.com/Cashop-Tech/cashop-cli/releases/download/admin-v${VERSION}/cashop-console-cli-${VERSION}.tar.gz"
...
FORMULA=Formula/cashop-console.rb
[ -f "$FORMULA" ] || { echo "formula not found at $FORMULA — create it first (see packages/cli-admin/scripts/cashop-console.rb.template)" >&2; exit 1; }
...
git add "$FORMULA"
git commit -m "cashop-console ${VERSION}"
git push
