...
import type { CartCountData } from '../../types/api.js';

const COUNT_PATH = '/trade/cashop-order-prod/api/cart/count';

const mod: CommandModule = {
  register(program: Command) {
    const cart = ensureGroup(program, 'cart');
    cart.command('count')
      .description('Get current cart item total count (for badge refresh)')
      ...
      .action(async function (this: Command) {
        ...
        const code = await runCmd(ctx, async () =>
          await gatewayRequest<CartCountData>(ctx.baseUrl, COUNT_PATH, {
